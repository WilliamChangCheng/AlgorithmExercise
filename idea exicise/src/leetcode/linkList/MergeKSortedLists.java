...
public class MergeKSortedLists {
     ...
     * The comparison cost will be reduced to O(logk) for every pop and insertion to priority leetcode.queue. But finding the node with the smallest value just costs O(1) time.
     * There are N nodes in the final linked list.
     * <p>
     * Space complexity :
     * O(n) Creating a new linked list costs O(n) space.
     * O(k) The code above present applies in-place method which cost O(1) space.
     * And the priority leetcode.queue (often implemented with heaps) costs O(k) space (it's far less than N in most situations).
     *
     * @param lists
     * @return
     */
